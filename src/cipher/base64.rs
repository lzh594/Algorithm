...
/// # Arguments
///
/// * `from`: Bytes: X and Y(Bits of those bytes will be signified using the names of their byte)
/// * `offset`:Offset: 4
///
/// returns: u8
///
...
///
/// ```
/// use algorithm::cipher::base64::base64_encode;
/// assert_eq!(base64_encode(b"\xc3\xc9\x18={\xc4\x08\x97wN\xda\x81\x84?\x94\xe6\x9e"),"w8kYPXvECJd3TtqBhD+U5p4=" );
/// ```
pub fn base64_encode(data: &[u8]) -> String {
    ...
    loop {
        let lower_bytes_index_to_encode = bits_encoded / 8usize;
        if lower_bytes_index_to_encode == data.len() {
            break;
        }
        let lower_bytes_to_encode = data[lower_bytes_index_to_encode];
        let upper_bytes_to_encode = if (lower_bytes_index_to_encode + 1) == data.len() { 0u8 } else { data[lower_bytes_index_to_encode + 1] };
        let bytes_to_encode = (lower_bytes_to_encode, upper_bytes_to_encode);
...
/// * `data`:
///
/// returns: Result<Vec<u8, Global>, (&str, u8)>
///
/// # Examples
///
/// ```
/// use algorithm::cipher::base64::base64_decode;
/// assert_eq!( base64_decode(&String::from("nw44vPXQLbQu1PA/j+cJey7/LzbLVFkhrjmC")).unwrap(),b"\x9f\x0e8\xbc\xf5\xd0-\xb4.\xd4\xf0?\x8f\xe7\t{.\xff/6\xcbTY!\xae9\x82".to_vec())
/// ```
pub fn base64_decode(data: &str) -> Result<Vec<u8>, (&str, u8)> {
    ...
    'decode_loop: loop {
        while collected_bits < 8 {
            if let Some(next_byte) = data_bytes.next() {
                if let Some(idx) = CHARSET.iter().position(|&x| x == next_byte) {
                    byte_buffer |= ((idx & 0b00111111) as u16) << (10 - collected_bits);
                    collected_bits += 6;
                } else if next_byte == (PADDING as u8) {
                    collected_bits -= 2;
                } else {
                    return Err((
        ...
        output_bytes.push(((0b1111111100000000 & byte_buffer) >> 8) as u8);
